...
function safeRender(val: any): string {
    ...
    if (typeof val === "object") {
        try {
            // If it's the {month, year} object, format it
        ...
        <Card>
            <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex-1 pr-4">
                    ...
                    <CardDescription>
                        Manage contracts. The PDF is generated on-the-fly with current signatures.
                    </CardDescription>
                </div>
                <ContractFormDialog studentId={safeRender(studentId)} supervisors={allSupervisors || []} />
            </CardHeader>
            <CardContent>
                {(contracts || []).length === 0 ? (
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Badge variant={STATUS_COLORS[String(contract.status || "")] as any ?? "secondary"}>
                                                {safeRender(contract.status || "UNKNOWN")}
                                            </Badge>
                                        </div>
                                        <p className="text-xs text-muted-foreground">
                                            ...
                                            {" · "}Contract ID: <code className="font-mono">{safeRender(contract.id).slice(0, 8)}</code>
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {(contract.supervisors || []).map(cs => (
                                            ...
                                            >
                                                ...
                                                <span className="text-xs font-medium">{safeRender(cs.supervisor?.fullName)}</span>
