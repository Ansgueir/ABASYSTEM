...
import { Building2, Calculator, CheckCircle, Clock } from "lucide-react"
...
export function OfficeRevenueList({ entries }: OfficeRevenueListProps) {
    ...
    return (
        <Card className="overflow-hidden border border-border shadow-sm rounded-xl bg-white">
            {/* ── HEADER (Aligned with Student Header Sizing) ── */}
            ...
            <CardContent className="p-4 space-y-4">
                 {/* ── Settlement History (Table matching Student Paid Table) ── */}
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="border-b bg-muted/40 text-muted-foreground">
                                    ...
                                    <th className="text-left py-2 px-4 font-bold uppercase text-[9px]">Math</th>
                                ...
                                {entries.map(entry => (
                                    <tr key={entry.id} className="hover:bg-muted/10 transition-colors">
                                        ...
                                        <td className="py-2.5 px-4">
                                            <Popover>
                                                <PopoverTrigger className="text-slate-400 hover:text-primary flex items-center gap-1 text-[9px] uppercase font-bold tracking-widest transition-colors">
                                                    <Calculator className="h-3 w-3" /> Logic
                                                </PopoverTrigger>
                                                <PopoverContent className="w-64 p-3 rounded-xl border-border shadow-lg">
                                                    <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-2 border-b pb-1">Waterfall Logic</p>
                                                    <div className="space-y-1.5 text-[10px]">
                                                        <div className="flex justify-between">
                                                            <span>Gross from Student</span>
                                                            <span className="font-bold">{fmtUSD(entry.paymentFromStudent)}</span>
                                                        </div>
                                                        <div className="flex justify-between text-muted-foreground">
                                                            <span>Sup. Commission</span>
                                                            <span className="font-medium">− {fmtUSD(entry.supervisorPayout)}</span>
                                                        </div>
                ...
                {/* ── Revenue Summary (Match Student Contract Summary) ── */}
