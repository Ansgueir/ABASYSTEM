                ...
                for (const newUser of (newUsers ?? [])) {
                    ...
                    const user = await tx.user.create({
                        data: {
                            ...
                            student: {
                                create: {
                                    fullName: newUser.fullName, bacbId: newUser.bacbId ?? "", phone: newUser.phone ?? "", email,
                                    startDate: newUser.startDate ? new Date(newUser.startDate) : new Date(0),
                                    endDate: newUser.endDate ? new Date(newUser.endDate) : null,
                                    credential: (newUser.credential || "NO_CREDENTIAL") as any,
                                    level: (newUser.level || "BCBA") as any,
                                    school: "Imported", city: "N/A", state: "FL", totalMonths: newUser.totalMonths ?? 0,
                                    vcsSequence: newUser.vcsSequence ?? null, assignedOptionPlan: (newUser.optionPlan || "A") as any,
                                    regularHoursTarget: newUser.regularHoursTarget ?? null, concentratedHoursTarget: newUser.concentratedHoursTarget ?? null,
                                    independentHoursTarget: newUser.independentHoursTarget ?? null, totalAmountContract: newUser.totalAmountContract ?? null,
                                    analystPaymentRate: newUser.analystPaymentRate ?? null, officePaymentRate: newUser.officePaymentRate ?? null,
